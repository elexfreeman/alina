...
window.ImageFunctions = (function ($, fabric) {
    ...
    $(document).ready(function() {
         ...
         */
        createClickListener(divButtons.undoTab(), undoFunction);
        createClickListener(divButtons.redoTab(), redoFunction);
        ...
        $("#templateDropdownNewProject").change(function() {
            if($(this).find("option:selected").val() === "0,0,Add New Template") {
                ...
                $("#newTemplateWarning").css('display', 'inline-block');
            } else {
                ...
                $("#newTemplateWarning").css('display', 'none');
            }
        });
        ...
        $("#newTemplateWarning").attr('title',
            'The free version of the DigiWidgets plugin only supports one custom template\n' +
            'Once you make this template, you will not be able to modify it or create another');
    });
    ...
    $('#inline-btn').attr("src", DWWPSettings.path+'/remove.png');
        // Need to pass in the PostID so that we can update the content
    ...
    var model;
    self.applyTemplate = function (canvasWidth, canvasHeight, templateTitle) {
    ...
    self.disableMenuButtons = function () {
        ...
        $('#undoTab').prop('disabled',true).addClass('disabled');
        $('#redoTab').prop('disabled',true).addClass('disabled');
    ...
    self.enableMenuButtons = function () {
        ...
        $('#undoTab').prop('disabled',false).removeClass('disabled');
        $('#redoTab').prop('disabled',false).removeClass('disabled');
    ...
    self.toggleBorder = function () {
        ...
        borderToolOptionsButton.css({ top: borderToolPosition.top + 20, left: (borderToolPosition.left + borderToolButton.width() + 55) });
    ...
    function zoomObjectFunction(object, zoomValue) {
        ...
        /*
         * Creating shortcut for undo action
         */
        ...
        /*
         * Creating shortcut for redo action
         */
        ...
        /*
         * Undo the last action made on the canvas
         */
        var undoFunction = function () {
            if (commandHistory.length > 1) {
                undoArray.push(commandHistory.pop());
                var lastCommand = [];
                lastCommand = commandHistory.pop();
                commandHistory.push(lastCommand);
                //var helper = JSON.stringify(lastCommand);

                var helper = JSON.stringify(lastCommand);
                canvas.clear();
                //btn.style.visibility = 'hidden';
                loadFromTimestamp(commandHistory.length-1);
            }
        };

        /*
         * Redo function that pulls from undoArray and loads the JSON into the canvas
         */
        var redoFunction = function () {
            if (undoArray.length > 0) {
                var lastCommand = [];
                lastCommand = undoArray.pop();
                commandHistory.push(lastCommand);
                    ...
                    if (msg !== 'false') {
                        ...
                        if (firstImage === true) {
                            ...
                            $.post(ajaxurl, data, function(msg) {
                                ...
                                canvas.loadFromJSON(finalData, renderFunction);
                            });
                        }
                        else {
        ...
        function getCommandHistory(image, applyTemplate) {
            ...
            $.post(ajaxurl, data, function(msg) {
                ...
                if(applyTemplate !== 0){
                    self.applyTemplate(applyTemplate[0],applyTemplate[1],applyTemplate[2]);
                }
            });
        }
        ...
        var addImageRenderList = function () {
            ...
            self.saveCommandHistory('Add Image');
            if(templateSettings.title !== ""){
                self.applyTemplate(templateSettings.width,templateSettings.height,templateSettings.title);
            }
        };
    ...
    self.selectExistingProject = function(projectLink) {
        ...
        self.cancelExistingProject();
        self.enableToolSetButtons();
        addImageFromURL(imageLink, true, templateSettings);
    };

    self.cancelExistingProject = function() {
        //Have to completely remove table and rebuild it everytime. This allows us to stay up to date with new images.
        var existingProjectContainer = document.getElementById("existingProjectContainer");
        $('#existingProjectContainer').css('display', 'none');
        $('#wpfooter').css('margin-top', '0px');

        while (existingProjectContainer.firstChild) {
            existingProjectContainer.removeChild(existingProjectContainer.firstChild);
        }
    };
    
    self.clickExistingProject = function() {
        var existingProjectContainer = document.getElementById("existingProjectContainer");
                        ...
                        $.post(ajaxurl, data, function(msg) {
                            ...
                            $.post(ajaxurl, data, function(data) {
                                if ($('#templateDropdown').find('option').length > 0) {
                                    ...
                                    $('#templateDropdownNewProject').empty();
                                }
   ...
   //Right Arrow
    ...
    var optionCheck = function(){
        ...
        if(option) {
            option = option.split(',');
            self.applyTemplate(option[0],option[1],option[2]);
        }
    };
        ...
        if (imageLink !== "") {
            var existingProjectContainer = document.getElementById("existingProjectContainer");
            existingProjectContainer.style.visibility = 'hidden';
        ...
        $.post(ajaxurl, data, function(data) {
            ...
            if (imageLink === "") {
                //Disabling import button for now until it is fixed
                $("#import").attr("disabled", "disabled").off('click');
                $('#templateDropdown').prop('disabled',true).addClass('disabled');
