...
class Links_List_Table extends WP_List_Table {
        ...
        if ( $which == "top" ){
            ...
            echo '<div class="wrap">
                <h2 style="display: inline-flex;">DigiWidgets Projects
                    <div id="newProject" onclick="ImageFunctions.clickNewProject()" style="width: 84px; margin-left: 20px;" class="add-new-h2">Add New</div>
                </h2></div>';
        }
        if ( $which == "bottom" ){
    ...
    function prepare_items() {
        ...
        $query = "SELECT projectTitle, pictureID, guid, modified FROM $table_name INNER JOIN $tablePost ON pictureID=ID ORDER BY pictureID DESC";
    ...
    function display_rows() {
        ...
        if(!empty($records)){foreach($records as $rec){
            //Open the line
            $selectCommand = "ImageFunctions.selectExistingProject('".$rec->guid."')";
            ...
            foreach ( $columns as $column_name => $column_display_name ) {
                ...
                switch ( $column_name ) {
                    ...
                    //case "col_template_select": echo '<td '.$attributes.'><strong><div id="select" title="Select" onclick="'.$selectCommand.'">Select</div></strong></td>'; break;
                }
            }
            echo "<td><div onclick='ImageFunctions.deleteExistingProject(\"$rec->pictureID\")' class='dashicons dashicons-trash closeProject'></div></td>";
            //Close the line
            echo'</tr>';
        }}
